...
| 2026-08-15 | 성남시 시딩 (`/spec`→`/impl`, 실API로 스코프 1회 수정) | n/a | 0 | 1/1 | AC 8개 → 테스트 11개. 실API 검증 중 AC1 전제(주소 필드 존재)가 깨진 걸 발견해 명세 1회 개정. 실행: 2273건→1913건 upsert(고유좌표 68.6%, 원본 59%) |
...
### 시딩 기능에서 관찰된 것 (스코프가 실행 중 뒤집힌 사례)

- **`/spec`이 "테스트로 번역 가능"을 강제했는데도 API 스펙 자체가 틀렸다.** AC1은
  `REFINE_LOTNO_ADDR` 필드 존재를 전제했다 — 이건 명세 단계의 실수가 아니라, 원본 코드를
  참고할 때 그 필드가 "이 데이터셋의 실제 응답"이 아니라 "여러 데이터셋을 아우르는 후보
  목록의 일부"였다는 걸 놓친 것이다. 유닛 테스트(가짜 repository)는 이 문제를 못 잡는다 —
  가짜는 내가 준 대로만 응답하므로, 틀린 전제를 그대로 통과시킨다. **실제 API를 한 번
  호출해보고 나서야** 드러났다.
- **원본 DB를 직접 열어본 게 결정적이었다.** "원본은 이 문제를 어떻게 풀었나"를 코드
  추측이 아니라 `psql`로 실제 저장된 값(`address` 컬럼에 법정동명이 들어있음)과 집계
  (좌표 다양성 59%)로 확인했다. 문서·코드만 봤으면 `match_confidence` 컬럼이 실제로
  구현됐는지도 몰랐을 것이다(스펙 문서엔 있었지만 실제 스키마엔 없었다).
- **게이트는 이 문제를 하나도 못 잡는다.** 5종 체크(typecheck/lint/stylelint/test/build)는
  이 버그가 있는 상태에서도 전부 초록이었다 — 가짜 의존성으로 짠 단위 테스트가 "정상
  동작"을 증명하는 것과 "실제 데이터에 대해 옳다"를 증명하는 것은 다르다는 걸 보여주는
  사례. 이런 종류의 정합성은 실행해보는 것 말고는 검증 수단이 없다.

### offices API 작업에서 관찰된 것
