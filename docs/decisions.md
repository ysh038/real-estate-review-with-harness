...
### 2026-08-26 #10 — 범위를 MVP+Phase 1에서 원본 전체 캐치업으로 확장한다; 별점은 유지한다 (원본의 Phase 7 제거는 따르지 않는다)

- **배경**: 원본 저장소(`real-estate-agent-review`)가 이 PC에도 클론되어 있어
  `docs/product-spec.md`를 직접 대조했다. 원본은 Phase 1~13까지 구현돼 있다(Phase 6
  배포·10-B 실제 법적 문구만 미완). 이 저장소는 실험 설계상(`docs/experiment.md`, 결정
  #1) **MVP + Phase 1** 까지만 의도적으로 좁혀 구현했었다.
- **결정**: "하네스를 얹으면 원본만큼(혹은 그 이상) 개발이 가능한가"를 직접 확인하기 위해
  범위를 원본의 **Phase 1~13 전체 캐치업**으로 확장한다. 갭 목록은
  `docs/product-spec.md` TODO에 Phase 2~13으로 반영했다.
- **원본 Phase 7(별점 제거)과의 충돌**: 원본은 "부동산 도메인 특성상 별점이 허위·악의적
  평가 논란과 법적 분쟁 리스크를 높인다"는 이유로 Phase 7에서 별점을 컬럼까지 완전히
  제거했다. 이 저장소는 결정 #9에서 이미 별점을 유지하기로 했었는데, 원본이 나중에
  뒤집었다는 사실을 알고 난 뒤에도 **별점 유지를 재확인**한다.
- **대안**: 원본처럼 Phase 7을 따라가 별점을 제거한다(UI·API·DB 컬럼 전부 삭제).
- **근거**: 결정 #9의 근거(이미 여러 명세·API·UI에 걸쳐 구현·검증돼 제거 비용이 재작업
  수준으로 큼)가 여전히 유효하고, 사용자가 원본의 반대 결정을 인지한 상태에서도 유지를
  명시적으로 재확인했다. 원본을 그대로 복제하는 것이 실험 목적이 아니라, 하네스를 쓴
  결과물이 원본과 어떻게 달라지는지 관찰하는 것이 목적이므로 이 지점에서 독자 판단을
  유지하는 것도 유효하다.
- **영향**: Phase 8(상세 라우트)·9(마이페이지)·11(리뷰 UX)·12(콘텐츠 고도화) 작업에서
  원본 스펙의 별점 관련 요소(있다면 제거 방향)를 그대로 가져오지 않고, 이 저장소는 별점을
  유지하는 쪽으로 조정해서 구현한다.
...
### 실 카카오 로그인이 필요한 UI 브라우저 검증 — 누적 미완료 목록
...
### 추가 — `my-reviews-list`(덩이 I) `MyReviewsPanel` 시각 검증도 같은 이유로 미완료

- **상황**: 헤더의 "내 리뷰" 버튼 → `MyReviewsPanel`(백드롭 모달)까지는 로그인 상태여야
  도달한다. 위와 같은 자격증명 제약으로 실 브라우저에서 열어보지 못했다. Testing Library
  컴포넌트 테스트(DOM·ARIA·상호작용 9개)와 `next build`의 CSS 유효성 검사(백드롭
  `color-mix()` 표현 포함)로 대체 검증했다.
- **후속 조치**: 위 AC20~22와 한 번에 처리한다 — 다음에 실 로그인 세션이 있을 때
  "내 리뷰" 버튼 클릭 → 패널 열림 → 목록·숨김 표시·더보기·배경 클릭/ESC 닫힘까지
  한 번에 훑는다.
