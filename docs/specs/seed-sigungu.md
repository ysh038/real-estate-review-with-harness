...
- [x] AC1: 경기데이터드림 원천 레코드 1건(법정동명 포함) + 키워드 지오코딩 성공 좌표 →
      `TOfficeInsert` 1건으로 변환된다. `address` 컬럼에는 `LEGALDONG_NM`이 들어간다.
- [x] AC2: 지오코딩이 실패한(키워드 검색 결과가 법정동명과 매칭되지 않는) 레코드는
      **건너뛰고** 나머지는 계속 처리한다 — 하나의 실패로 전체 시딩이 죽지 않는다.
- [x] AC3: 원천 레코드에 대표자명이 없으면(빈 문자열) `ownerName: null` 로 매핑한다.
- [x] AC4: 같은 등록번호가 원천 목록에 중복으로 오면(페이지네이션 겹침 등) 1건으로 합쳐진다.

**실행 요약** (서비스 단위 테스트)

- [x] AC5: 실행이 끝나면 `{ fetched, upserted, skipped }` 요약을 반환한다.
- [x] AC6: 경기데이터드림 API가 재시도 후에도 실패하면(4xx/5xx) 에러를 던지고 종료한다 —
      부분 데이터로 조용히 끝나지 않는다.

**CLI**

- [x] AC7: `bun run --cwd apps/api seed:sigungu -- 수원시` 실행 시 인자로 받은 시군을
      `SEED_TARGET_SIGUNGU` 기본값보다 우선한다.
- [x] AC8: 실행 종료 시 콘솔에 `fetched/upserted/skipped` 요약을 출력한다.
...
## 실행 결과 (성남시, 2026-08-15)

```
fetched=2273 upserted=1913 skipped=360
```

DB 확인: 1913건 중 고유 좌표 1313개 (68.6%) — 같은 법정동 안에서도 대부분 서로
다른 위치로 흩어졌다. 원본 실행 결과(2273건 중 1346/2273=59%)보다 고유 좌표 비율이
높다. keyword 검색 쿼리를 "법정동명 + 사무소명"으로 고정한 것과 prefix 필터가
비교적 잘 작동한 것으로 보인다 — 원본처럼 복합 fallback을 안 쓰고도 나온 결과.
