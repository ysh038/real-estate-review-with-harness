...
**완료** (명세: `specs/legal-pages-and-footer.md` — AC1~10 전부 확인, 실 브라우저로
6개 페이지 전부 검증). 개인정보처리방침·오픈소스 목록은 placeholder가 아니라 이
저장소의 실제 동작·의존성 기준 사실 내용으로 채웠다(전문가 검토가 필요한 "계약
조항 문구"와는 다른 성격이라 판단).

- [x] 공통 푸터 + `/legal/*` 라우트 스캐폴딩(약관·개인정보처리방침·위치기반서비스
      이용약관·오픈소스 고지)
- [x] 각 페이지 "준비 중" placeholder + 최종 개정일 필드 (약관·위치기반서비스만 —
      개인정보처리방침·오픈소스는 실제 내용)
- [x] `/contact` 문의 채널
- [x] FAQ — placeholder 대신 이 저장소의 실제 기능 기준 실제 문답으로 작성(계약
      문구가 아니라 사용법 안내라 전문가 검토 대상이 아니라고 판단)
- [ ] (실배포 직전) 실제 법적 문구 작성 — 전문가 검토 필요, 지금 범위 밖

#### Phase 11 — 리뷰 신뢰도 & UX

**퍼머링크·신고 UI·정렬 완료** (명세: `specs/review-permalink-report-and-sort.md` —
AC1~12 전부 확인, Vitest 20건 신규 + 통합 테스트 + 브라우저 검증)

- [x] 개별 리뷰 퍼머링크 (Phase 8 선행 필요)
- [x] 신고 UI(버튼) — 사유 선택은 이 저장소의 API가 사유를 안 받아 제외
...
**12-C 정형 설문 완료** (명세: `specs/review-structured-survey.md` — AC1~12
전부 확인, Vitest 41건 신규/확장 + 브라우저 검증). 원본도 항목이 미확정
상태였던 것을 이 저장소가 먼저 확정·구현했다 — 전문성 평가(3단계)·하자 대응
경험(3지선다), 둘 다 원본에 없던 보기 문구라 독자적으로 새로 지었다(설계
메모 참고). 태그 6종과 겹치는 신호(허위매물·친절함 등)는 새 항목으로 만들지
않고 제외했다.

- [x] 12-C 정형 설문 — 전문성 평가 + 하자 대응 경험(선택 항목), DB·API·작성
      폼·카드 표시·임시저장 전부 반영
