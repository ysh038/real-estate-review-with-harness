...
export const createFakeReviewRepository = (
  ...
  insert: vi.fn(
    async (row): Promise<IReviewOwnedRow> => ({
      ...
      defectResponse: row.defectResponse,
      visitedYear: row.visitedYear,
      visitedMonth: row.visitedMonth,
      tags: row.tags,
      photos: row.photoKeys.map((storageKey: string) => ({ storageKey })),
      helpfulCount: 0,
      isHelpful: false,
    }),
  ),
  ...
  update: vi.fn(
    async (id, patch): Promise<IReviewOwnedRow> => ({
      ...
      defectResponse: patch.defectResponse,
      visitedYear: patch.visitedYear,
      visitedMonth: patch.visitedMonth,
      tags: patch.tags,
      photos: patch.photoKeys.map((storageKey: string) => ({ storageKey })),
      helpfulCount: 0,
      isHelpful: false,
    }),
  ),
